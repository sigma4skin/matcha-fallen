	...
	local function BuildSelfFilter()
		return GetCharacterParts(LocalPlayer.Character)
	end

	local function BuildVisFilter(TargetCharacter)
		local List = GetCharacterParts(LocalPlayer.Character)
	...
	local function BuildVisParams(TargetCharacter)
		local Params = RaycastParams.new()
		Params.FilterType = Enum.RaycastFilterType.Exclude
		Params.FilterDescendantsInstances = BuildVisFilter(TargetCharacter)
	...
	local function IsTargetVisible(Character, TargetPart)
		...
		local Params = BuildVisParams(Character)
	...
	local function IsVisibleFrom(Origin, TargetPart, TargetCharacter)
		...
		local Direction = TargetPart.Position - Origin
		local Params = BuildVisParams(TargetCharacter)
		local Result = Workspace:Raycast(Origin, Direction, Params)
	...
	local function DisableManip()
		...
		memory_write(getbase() + Cheat.Offsets.FFlags.PhysicsSenderMaxBandwidthBps, 38760)
		memory_write(getbase() + Cheat.Offsets.FFlags.DataSenderRate, 60)
	end
	...
	GunModsSec:Toggle("Instant Bullet", false, function(Bool)
		if Bool then
			EnableInstantBullet()
		else
	...
	GunModsSec:Toggle("Fire Rate", false, function(Bool)
		Flags.FireRate = Bool

		if Bool then
			ApplyFireRate(Flags.FireRateMult)
		else
			ApplyFireRate(1)
		end
	end)

	GunModsSec:Slider("Fire Rate Multiplier", 1, 0.1, 1, 1.5, "x", function(V)
		Flags.FireRateMult = V

		if Flags.FireRate then
			ApplyFireRate(V)
		end
	end)

	GunModsSec:Toggle("No Sway", false, function(Bool)
		if Bool then
			EnableNoSway()
		else
			DisableNoSway()
	...
	GunModsSec:Toggle("Thick Bullet", false, function(Bool)
		Flags.ThickBullet = Bool
	end)

	GunModsSec:Slider("Thickness", 1, 0.1, 1, 4, "x", function(V)
	...
	do
		RunService.Heartbeat:Connect(function()
			...
			if Flags.ThickBullet then
				for _, Player in Players:GetPlayers() do
					if Player.Address == LocalPlayer.Address then
						continue
					end
